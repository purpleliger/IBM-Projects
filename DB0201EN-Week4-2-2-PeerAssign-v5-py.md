...
%sql ibm_db_sa://kqm55602:x8ggl2f%2Bw02vw0kk@dashdb-txn-sbox-yp-dal09-04.services.dal.bluemix.net:50000/BLUDB
```




    'Connected: kqm55602@BLUDB'
...
```
     ...
     * ibm_db_sa://kqm55602:***@dashdb-txn-sbox-yp-dal09-04.services.dal.bluemix.net:50000/BLUDB
    Done.
...
```python
%sql select COUNT(ID) as crimes_with_arrests from CHICAGO_CRIME_DATA where arrest = 'TRUE'
```
...
```python
%sql select DISTINCT(primary_type) as unique_crimes_at_gas_stations from CHICAGO_CRIME_DATA where location_description = 'GAS STATION'
```
...
<table>
    <tr>
        <th>unique_crimes_at_gas_stations</th>
    </tr>
    <tr>
...
```python
%%sql select AVG(safety_score) as average_school_safety_score
from CHICAGO_PUBLIC_SCHOOLS
```
...
```python
#2) save the average enrollment per area numbers into a tuple of (community_area_name, average_college_enrollment)
%%sql select community_area_name, AVG(college_enrollment) as average_college_enrollment
from CHICAGO_PUBLIC_SCHOOLS
#1) group by community area names and find average enrollment in that area
group by community_area_name
#3) sort the tuple by average_college_enrollment in descending order, show only top 5 tuples
